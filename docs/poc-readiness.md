# POC Readiness and Evidence v4.4.11
...
Run the check with any accessible Python 3.14+ interpreter; the installation
source is not prescribed. The check is read-only and does not deploy schema
objects:
...
```bash
"$PYTHON_BIN" scripts/poc_evidence.py \
...
```bash
"$PYTHON_BIN" scripts/support_bundle.py \
...
master-key file, and exact release archive. Rollback stops v4.3.0 services and
restores the coordinated v4.1.x snapshot/profile. External side effects and
completed credential rotations require separate operational handling.
