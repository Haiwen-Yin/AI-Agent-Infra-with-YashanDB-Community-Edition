# AI Agent Infra with YashanDB — 社区版 v4.4.11
...
> v4.4.2 Embedding 与图工程运行：管理员先测试模型服务，平台自动识别向量维度并原子化维护 Profile、Contract、默认 Space 与 Binding；统一归一化规则由平台契约强制执行，后续结果为只读状态，不再要求手工创建契约、空间或迁移任务。知识清单与图投影按已认证主体及其有效可见范围查询；全局可见范围不会向 YashanDB 提交未使用的主体参数。Graph Production Profile 按能力控制 Graph Core、检查、A2A、OTLP、回放和动态迁移；稳定能力可生产使用，其余默认关闭或受控启用。
...
> **注意**：YashanDB 不支持引用分区（Reference Partitioning）和 JSON 关系对偶视图（JRD）。v4.1 的关系型图查询保留共享关系边实现；v4.2.1 实验性 Graph Engineering 另外使用 YashanDB 原生 Property Graph 投影，运行时状态仍以关系型 Graph 表为权威。
...
受限输入和短期 Lease Token，不会获得 Schema Owner 凭证；过期或撤销的
fencing token 不能覆盖更新后的执行结果。成功完成会保存请求摘要，同一 token
对同一输出和证据的重试只返回原有 Checkpoint，不会重复写入状态或下游
Transition；冲突输出仍会被拒绝。
