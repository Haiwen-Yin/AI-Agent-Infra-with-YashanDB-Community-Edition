# AI Agent Infra with YashanDB — 企业版 v4.0.0
...
> **关于本项目**：YashanDB 适配版本是为崖山数据库原生编写的全新适配层，基于统一源码仓库（`shared/` + `adapters/yashandb/`）构建，使用 yaspy 1.2.1 Python 驱动。本版本与 Oracle / PostgreSQL 版本功能对等、API 一致，但底层安全模型、SQL 方言、PL/SQL 包实现均针对 YashanDB 独立实现。
...
项目最初基于 Oracle 数据库构建，随后在 v3.6.1 扩展至 PostgreSQL。**v3.10.2 起，项目原生适配崖山数据库（YashanDB）23.5+**，使用 yaspy 1.2.1 Python 驱动，成为支持三种数据库的多版本基础设施。YashanDB 适配版本针对崖山数据库的 VECTOR 向量列、内置加密包、End User 管理、定时任务等能力独立实现，与 Oracle / PostgreSQL 版本功能对等、API 一致，但底层 SQL 方言、PL/SQL 包、安全模型均针对 YashanDB 独立设计。

v4.0.0 起，项目重构为统一源码仓库（`shared/` 共享代码层 + `adapters/<db>/` 数据库适配层），通过 build.py 一键打包 6 个版本（3 种数据库 × 社区版/企业版）。

### 版本演进

> 以下仅列出 YashanDB 适配版本的相关里程碑。v3.10.2 之前的项目历史（v1.0.0 ~ v3.10.1）基于 Oracle / PostgreSQL，详见 CHANGELOG.md。

| 版本 | 日期 | 里程碑 |
|------|------|--------|
| **v4.0.0** | 2026-07-19 | 基于 Spec-Driven Development 进行项目结构重构：统一源码仓库（shared/ + adapters/）、build.py 一键打包 6 个版本、参数化测试框架、OpenSpec 规格守护。新增 SDD 功能：MCP 动态工具加载、Spec 版本管理、Harness 继承、DAG 可视化、审批 SLA、事件死信队列。搜索增强：搜索解释、图算法(PageRank/最短路径/社区检测)、批量嵌入、Agent 自动恢复。安全加固：config.json 排除打包、敏感信息自动脱敏、首次启动配置向导。YashanDB 适配增强：yaspy 驱动 + 客户端库去重（41MB -> 25MB）、install_yaspy.sh 符号链接重建、SKILL.md 部署指南、NOTICE 动态生成 |
| **v3.10.2** | 2026-07-17 | **YashanDB 首次适配**：yaspy 1.2.1 驱动适配、connection.py VECTOR array.array -> string 转换（规避 GC segfault）、1_schema.sql 适配（无引用分区/无 JSON_OBJECT/无内联 FK/无 LOCAL 索引）、deploy_yashandb.py 纯 Python 部署脚本、install_yaspy.sh 驱动安装、vendor/yaspy/ 离线驱动包。与 Oracle / PostgreSQL 版本功能对等、API 一致 |
...
| **Role-Based Access Control** | 每张表通过 End User 角色 + GRANT 权限矩阵实现行级隔离 |
...
> **注意**：YashanDB 不支持 Oracle 的 JSON 关系对偶视图（JRD）。本项目在 YashanDB 上使用普通视图 + JSON 函数替代。
...
加密凭证系统确保敏感信息在静态存储时始终处于加密状态，采用本地文件加密（connection_crypto）和数据库内加密（YashanDB 内置加密包）双轨方案，覆盖数据库连接、LDAP 凭证和 Agent 凭证三大场景。
...
| **L4 Role-Based Access Control** | End User 角色 + GRANT 权限矩阵 + End User 隔离 | 声明式行/列/单元级访问控制 + 强制访问控制 + 零信任 — ✅ **策略已部署并生效** |
| **L5 统一审计** | 审计策略 | 审计所有绕过 API 的直接操作 |
| **L6 凭证过滤** | save_context() 自动脱敏 | 防止凭证泄露到上下文存储 |

> **L4 Role-Based Access Control 执行状态**
>
> YashanDB 通过 **Direct Logon with Local End Users** 模式在数据库层完全执行：
> - 每个 Pool Agent 对应一个 End User（`UPPER(REPLACE(agent_id, '-', '_'))`），通过 `END_USER_MANAGER.ensure_end_user` 创建
> - Portal 用户以 End User 身份连接 → 通过 End User 上下文自动过滤可见行
> - Admin Dashboard 使用 AIADMIN 连接池（schema owner，不受 End User 限制）
> - `connection.py` 自动路由：`set_agent_context()` → End User 连接；无上下文 → AIADMIN 池
> - 角色权限矩阵覆盖 Portal 写操作授权及协作组访问授权
>
> 验证结果（AGENT_001 vs AIADMIN）：
>
> | 表 | AIADMIN（全量） | AGENT_001（End User） | 过滤率 |
> |----|----------------|---------------------|--------|
...
> YashanDB 使用普通视图 + JSON 函数替代 Oracle 的 JRD 双重视图。
