...
export default function ToolsCatalog({ text }: { text: (zh: string, en: string) => string }) {
  ...
  const setExposure = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (!selected || busyId) return;
    const id = catalogId(selected, "tools"); const data = new FormData(event.currentTarget);
    const current = selected.mcp_exposed === "Y";
    setBusyId(id); setError(""); setNotice("");
    try {
      const response = await fetch(`/api/tools/${encodeURIComponent(id)}/mcp-exposure`, { method: "POST", credentials: "same-origin", headers: importHeaders(), body: JSON.stringify({ expected_exposed: current, exposed: !current, reason: String(data.get("reason")) }) });
      if (!response.ok) throw new Error(response.status === 409 ? text("工具状态已变化，请刷新后重试。", "Tool state changed; refresh before retrying.") : text("MCP 开放设置未保存，请检查操作权限与服务状态。", "MCP exposure was not saved; check permissions and service status."));
      setSelected({ ...selected, mcp_exposed: current ? "N" : "Y" }); await load();
      setNotice(text("MCP 开放设置已更新并记录审计。", "MCP exposure updated and audited."));
