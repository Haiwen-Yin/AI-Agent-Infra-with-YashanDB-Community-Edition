...
export default function GraphPage({ lang, text, onNotice }: Props) {
  ...
  return <section>
    ...
    {loading ? <div className="empty-state cx-data-loading" role="status"><span className="cx-loader spinner" /><span>{text("正在读取数据库", "Reading database")}</span></div> : <>
      ...
      {view === "definitions" && <section className="info-panel"><div className="panel-title"><h2>{text("图定义", "Graph definitions")}</h2></div>{table(["ID", text("名称", "Name"), text("说明", "Description"), text("所有者", "Owner"), text("状态", "Status")], graphs.map((row) => [detailButton(row, field(row, ["graph_id", "definition_id", "id"])), display(field(row, ["graph_name", "name", "title"])), display(field(row, ["description", "summary"])), display(field(row, ["owner_ref", "owner"])), statusLabel(field(row, ["status"]), text)]), text("暂无图定义", "No Graph definitions"))}</section>}
      {view === "types" && <section className="info-panel"><div className="panel-title"><h2>{text("图类型", "Graph types")}</h2></div>{table([text("类型", "Type"), text("名称", "Name"), text("版本", "Version"), text("状态", "Status")], types.map((row) => [graphTypeKind(field(row, ["type_kind", "kind", "type", "graph_type"])), graphTypeName(field(row, ["type_name", "name", "title"])), display(field(row, ["type_version", "version"])), statusLabel(field(row, ["status"]), text)]), text("暂无图类型", "No Graph types"))}</section>}
      {view === "runs" && <section className="info-panel"><div className="panel-title"><h2>{text("运行记录", "Graph runs")}</h2></div>{table(["ID", text("图版本", "Graph version"), text("状态", "Status"), text("更新时间", "Updated")], runs.map((row) => [detailButton(row, field(row, ["run_id", "id"])), String(field(row, ["graph_version_id", "version_id"])), statusLabel(field(row, ["status"]), text), String(field(row, ["updated_at", "created_at"]))]), text("暂无运行记录", "No runs"))}</section>}
