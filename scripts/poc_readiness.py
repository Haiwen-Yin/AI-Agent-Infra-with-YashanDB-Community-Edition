...
def _version(override: str | None = None) -> str:
    if override:
        return override
...
def check_target(database: str, config_path: Path | None, enterprise: bool = True) -> dict[str, Any]:
    ...
    if not result["runtime"]["python_ok"]:
            result["remediation"].append("Run the POC with an accessible Python 3.14 or newer interpreter.")
...
def build_report(configs: dict[str, Path | None], enterprise: bool = True,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run non-destructive POC readiness checks.")
    ...
    parser.add_argument(
        "--version",
        help="release version represented by this evidence (defaults to the package/source version)",
    )
