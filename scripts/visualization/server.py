...
class VisHandler(BaseHTTPRequestHandler):
    ...
    def _handle_chat_stream(self, message, agent_id, user_id, workspace_id, sess, ctx_id):
        ...
        # Check if LLM is configured before attempting stream
        from lib.config import get_config as _gc
        _llm_cfg = _gc().llm
        _llm_ready = bool(_llm_cfg.api_url and _llm_cfg.model)
    ...
    try:
        ...
        for line in resp:
            ...
            try:
                ...
                token = delta.get("content") or ""  # Skip reasoning_content in streaming
                if token:
                    yield token
...
def _call_llm(messages, model=None):
    ...
    try:
        ...
        if llm.api_key:
            _headers["Authorization"] = f"Bearer {llm.api_key}"
