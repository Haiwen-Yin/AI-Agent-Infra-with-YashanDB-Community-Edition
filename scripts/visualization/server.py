"""AI Agent Infra v4.0.0 - Enterprise Edition - Web Visualization Server
...
def _get_session(request_handler):
    ...
    if not sess:
        return None
    cfg = _load_server_config()
    timeout = getattr(cfg, 'session_timeout', 3600)
...
class VisHandler(BaseHTTPRequestHandler):
    ...
    def _handle_login(self):
        ...
        self.send_header('Content-Type', 'application/json')
        self.send_header('Set-Cookie', '{}={}; Path=/; HttpOnly; Max-Age=3600; SameSite=Lax'.format(_get_cookie_name(), session_id))
    ...
    def _handle_portal_register(self):
        try:
            ...
            pool_agent = connection.execute_query_one(
                "SELECT AGENT_ID FROM AGENT_REGISTRY WHERE STATUS = 'ACTIVE' AND AGENT_ID LIKE 'AGENT_POOL_%' ORDER BY AGENT_ID"
            )
            if pool_agent:
    ...
    def _serve_template(self, filename):
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                html = f.read()
            html = html.replace('4.0.0', VERSION)
