"""AI Agent Infra v4.0.1 - Enterprise Edition - Web Visualization Server
...
def _get_or_assign_portal_agent(user_id):
    ...
    if assigned:
        return agent_api.get_agent(assigned['agent_id']) or assigned
    return agent_api.assign_random_pool_agent(str(user_id), node_id)
    ...
    def _serve_template(self, filename):
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                html = f.read()
            timeout = _session_timeout()
            html = html.replace('4.0.1', VERSION)
