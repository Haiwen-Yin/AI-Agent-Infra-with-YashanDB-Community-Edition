"""AI Agent Infra v4.2.0 - Community Edition - Web Visualization Server
...
if getattr(edition_features, 'GRAPH_ENGINEERING_ENABLED', False):
    from lib import graph_definition_api, graph_compiler, graph_runtime, graph_worker, graph_event_api, graph_adapter, graph_compat
else:
    graph_definition_api = graph_compiler = graph_runtime = graph_worker = graph_event_api = graph_adapter = graph_compat = None
    ...
    def _handle_api_get(self, path, qs):
        try:
            if path == '/api/health':
                ...
                'evaluations', 'interventions', 'joins', 'branches', 'waits', 'migrations',
            }:
    ...
    def _serve_template(self, filename):
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                html = f.read()
            timeout = _session_timeout()
            html = html.replace('4.2.0', VERSION)
            html = html.replace('2026-07-25', os.environ.get('AI_AGENT_RELEASE_DATE', ''))
