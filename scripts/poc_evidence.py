...
def build_report(manifest_path: Path, clean_path: Path, mode_path: Path,
    ...
    # POC acceptance is a bounded customer workflow, not a release-maturity
    # decision. Capacity and recovery may still block the production profile
    # while the five agreed POC items are independently accepted.
    ...
    return {
        ...
        "passed": passed,
        "release_gate_status": str(manifest.get("release_status") or "UNVERIFIED").upper(),
        "mandatory": mandatory,
