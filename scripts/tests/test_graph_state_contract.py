...
def test_state_rejects_non_finite_numbers_before_persistence():
    _, errors = prepare_output({"value": math.inf}, {"type": "object"})
    assert any(item["code"] in {"NONFINITE_NUMBER", "STATE_NOT_JSON"} for item in errors)


def test_state_schema_can_close_object_fields_explicitly():
    _, errors = prepare_output(
        {"result": "ok", "unexpected": True},
        {
            "type": "object",
    ...
    assert any(item["code"] == "ADDITIONAL_PROPERTY_FORBIDDEN" for item in errors)
