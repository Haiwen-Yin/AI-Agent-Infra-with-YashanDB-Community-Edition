"""AI Agent Infra v4.4.13 - Pytest Configuration and Shared Fixtures
...
# Make the bundled test suite self-contained after build.py moves it under
# scripts/tests.  This also lets source tests import the adapter-neutral lib
# package without relying on an ambient PYTHONPATH.
...
# These legacy suites exercise the concrete adapter directly and are kept out
# of the adapter-neutral source run.  They are executed from each generated
# edition (or from the source tree with AIAGENT_SOURCE_ADAPTER explicitly set)
# where the selected driver and a real test database are available.
SOURCE_DATABASE_TEST_MODULES = {
    "test_admin_agent.py", "test_agent.py", "test_collab.py", "test_connection.py",
    "test_credential.py", "test_embedding.py", "test_graph.py", "test_harness.py",
    "test_knowledge.py", "test_loop_api.py", "test_memory.py", "test_search_api.py",
    "test_skill.py", "test_spec.py", "test_unified_search.py", "test_workspace.py",
    "test_all.py",
}
...
def pytest_report_header(config) -> List[str]:
    ...
    lines = [
        "AI Agent Infra v4.4.13 - parameterized DB fixtures",
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        if (
            not PACKAGED_EDITION
            and not os.environ.get("AIAGENT_SOURCE_ADAPTER")
            and Path(str(item.fspath)).name in SOURCE_DATABASE_TEST_MODULES
        ):
            item.add_marker(pytest.mark.skip(reason="concrete adapter/database required; run the generated edition suite"))
        backend_params = [
