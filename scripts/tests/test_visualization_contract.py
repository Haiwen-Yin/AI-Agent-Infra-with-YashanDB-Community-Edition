...
def _react_ui_source():
    scripts_root = TEMPLATES_DIR.parent.parent
    source = scripts_root / "web" / "src" / "App.tsx"
    if source.is_file():
        return source.read_text(encoding="utf-8"), True
    assets = scripts_root.parent / "web" / "dist" / "assets"
    return "\n".join(path.read_text(encoding="utf-8") for path in assets.glob("*.js")), False


def _react_css_source():
    scripts_root = TEMPLATES_DIR.parent.parent
    source = scripts_root / "web" / "src" / "app.css"
    if source.is_file():
...
def test_portal_login_uses_identity_mfa_and_csrf_session_contract():
    server = SERVER_PATH.read_text(encoding="utf-8")
    login = (TEMPLATES_DIR / "portal_login.html").read_text(encoding="utf-8")
    chat = (TEMPLATES_DIR / "portal_chat.html").read_text(encoding="utf-8")
    assert "identity_api.authenticate_local(username, password)" in server
    assert "governed_contracts.mfa_admission_decision" in server
...
def test_dashboard_mfa_is_explicitly_configured_after_login():
    ...
    lifecycle = (TEMPLATES_DIR.parent.parent / "lib" / "security_lifecycle.py").read_text(encoding="utf-8")
    assert "管理员可登录后在用户管理中注册因子并配置是否强制" in ui
    assert "an active MFA factor is required before MFA can be enforced" in lifecycle
    assert "AND FACTOR_TYPE = 'TOTP' AND STATUS = 'PENDING'" in lifecycle
    confirm = app.split('def user_mfa_confirm(', 1)[1].split('@app.post("/api/users/{principal_id}/mfa/recovery-codes")', 1)[0]
    assert "identity_api.set_session_mfa_level" in confirm


def test_react_dashboard_keeps_navigation_and_session_controls_mounted():
    ui, is_source = _react_ui_source()
    css = _react_css_source()
    assert "pushState" in ui and "/app/" in ui
...
def test_react_dashboard_restores_database_graph_visualization():
    ui, _ = _react_ui_source()
    assert "/static/vis-network.min.js" in ui
    assert "/api/graph/all" in ui
    assert "知识图谱" in ui and "记忆图谱" in ui and "实体关系探索" in ui
    assert "knowledge" in ui and "memory" in ui
    assert "Search nodes" in ui


def test_react_dashboard_restores_admin_operational_surfaces():
    ui, _ = _react_ui_source()
    for endpoint in (
        "/api/monitor/agents", "/api/monitor/metrics", "/api/skill/create",
        "/api/branch/fork", "/api/loops/create", "/api/loops/runs/start",
    ):
        assert endpoint in ui
    assert "Legacy inventory (read-only)" in ui


def test_react_dashboard_exposes_switchable_views_and_dismissible_details():
    ui, is_source = _react_ui_source()
    css = _react_css_source()
    if is_source:
        ...
        assert "Escape" in ui
    assert "Branch relationships" in ui
    assert "Entity relationships" in ui and "Graph runs" in ui
    assert "Node type" in ui and "Relationship type" in ui
    assert "detail-backdrop" in ui
    assert "view-toggle" in css and "detail-backdrop" in css


def test_react_dashboard_has_explicit_database_loading_motion():
    ui, is_source = _react_ui_source()
    css = _react_css_source()
    assert "cx-data-loading" in ui and "cx-loader" in ui and "loading-dots" in ui
    assert ("@keyframes cx-dot" in css) if is_source else ("cx-dot" in css)


def test_react_dashboard_restores_skill_loop_channel_and_approval_controls():
    ui, is_source = _react_ui_source()
    if is_source:
        for endpoint in (
            "/api/skill/${encodeURIComponent(selected)}/update",
            "/api/skill/${encodeURIComponent(selected)}/upload",
            "/api/loops/runs/${action}",
        ):
            assert endpoint in ui
    else:
        assert "/api/skill/" in ui and "/update" in ui and "/upload" in ui
        assert "/api/loops/runs/" in ui
        assert all(label in ui for label in ("Update metadata", "Upload resource", "Pause", "Resume", "Stop"))
    assert "/api/channels" in ui
    assert "approval_status" in ui
    assert "Create Channel" in ui
    assert "Channel chat" in ui and "Channel management" in ui
    assert "Choose file" in ui and "No file selected" in ui
    assert "visually-hidden-file" in ui
    assert "cx-session-refresh" in ui


def test_global_relationship_graph_keeps_edge_endpoints_and_all_entity_types():
    server = SERVER_PATH.read_text(encoding="utf-8")
    graph_all = server.split("def _graph_all():", 1)[1].split("\ndef _get_tags_for_entities", 1)[0]
    assert "endpoint_ids" in graph_all
    assert "entity_id_expression" in graph_all
    assert "CAST(ENTITY_ID AS TEXT)" in graph_all
    assert "SELECT DISTINCT ENTITY_TYPE" in graph_all
    assert "str(src) not in known_nodes" in graph_all


def test_portal_chat_stream_is_not_buffered_by_compatibility_layers():
    web_app = (TEMPLATES_DIR.parent.parent / "web_app.py").read_text(encoding="utf-8")
    server = SERVER_PATH.read_text(encoding="utf-8")
    assert 'normalized == "/portal/api/chat/send"' in web_app
    assert "return await _legacy_stream_dispatch(" in web_app
    assert "class _LegacyStreamWriter:" in web_app
    assert "StreamingResponse(" in web_app
    assert "for raw_line in resp:" in server
    assert "resp.read(4096)" not in server
...
def test_portal_exit_waits_for_agent_release_before_redirect():
    ...
    assert "await portalFetch('/portal/api/agent/release'" in release
...
def test_react_dashboard_owns_theme_profile_metrics_and_collaboration_gate_labels():
    script = (TEMPLATES_DIR.parent / "static" / "chuanxu.js").read_text(encoding="utf-8")
    app, is_source = _react_ui_source()

    theme_toggle = script.split("function addThemeToggle()", 1)[1].split(
        "function updateBrand()", 1
    )[0]
    assert 'document.getElementById("root")' in theme_toggle
    if is_source:
        assert 'data.agents?.total' in app
        assert 'data.sessions?.active' in app
        assert 'data.tasks?.running_plans' in app
        assert 'api<Row>("/api/runtime/profile")' in app
        assert 'title={text("协作断点"' not in app
        monitor_route = app.split('if (page === "monitor")', 1)[1].split(
            'if (page === "graph")', 1
        )[0]
        monitor_page = app.split("function MonitorPage(", 1)[1].split(
        ...
        assert 'endpoint: "/api/workspaces?view=summary"' in app
    for label in (
        "Total Agents",
        "Active Sessions",
        "Running Task Plans",
...
def test_workspace_summary_mode_avoids_per_workspace_detail_queries():
    server = SERVER_PATH.read_text(encoding="utf-8")
    dispatch = server.split("elif path == '/api/workspaces':", 1)[1].split(
        "elif path == '/api/specs':", 1
    )[0]
    handler = server.split("def _api_workspaces(self, summary_only=False):", 1)[1].split(
        "def _api_specs", 1
    )[0]

    assert "view" in dispatch and "summary_only=" in dispatch
    assert "if summary_only:" in handler
    assert handler.index("if summary_only:") < handler.index("for ws in workspaces:")
    summary = handler.split("if summary_only:", 1)[1].split(
        "workspaces = connection.execute_query(", 2
    )[1].split("self._send_json", 1)[0]
    assert "workspace_id, workspace_name, status, updated_at" in summary
    assert "summary" not in summary.lower()
...
def test_enterprise_navigation_and_language_controls_match_dashboard_contract():
    if not all((TEMPLATES_DIR / name).exists() for name in ("approvals.html", "audit.html")):
        return
    approvals = (TEMPLATES_DIR / "approvals.html").read_text(encoding="utf-8")
    audit = (TEMPLATES_DIR / "audit.html").read_text(encoding="utf-8")
...
def test_audit_event_details_use_row_expansion_without_detail_column():
    if not (TEMPLATES_DIR / "audit.html").exists():
        return
    audit = (TEMPLATES_DIR / "audit.html").read_text(encoding="utf-8")
...
def test_experimental_graph_filters_cover_registered_and_node_types():
    graph = (TEMPLATES_DIR / "graph.html").read_text(encoding="utf-8")
    for marker in (
        'id="executionGraphTypeFilters"',
        'id="executionNodeTypeFilters"',
        'id="executionGraphNetwork"',
        "registeredGraphTypes",
        "executionGraphTypeKeys",
        "executionNodeMatchesFilters",
        "executionEdgeMatchesFilters",
...
def test_experimental_graph_risk_levels_are_localized():
    graph = (TEMPLATES_DIR / "graph.html").read_text(encoding="utf-8")
    assert "riskLevelLabels={LOW:['低','Low'],MEDIUM:['中','Medium'],HIGH:['高','High']}" in graph
    assert "function updateDefinitionRisk(level)" in graph
    assert "风险等级: " in graph
    assert "Risk level: " in graph


def test_execution_graph_preview_is_scrollable_and_not_clipped():
    css = (TEMPLATES_DIR.parent / "static" / "pages" / "graph.css").read_text(encoding="utf-8")
    assert ".content-area.graph-page-content{overflow-x:hidden!important;overflow-y:auto!important}" in css
    assert ".graph-view#graph-view-definitions{flex:0 0 auto}" in css
    assert ".graph-definition-layout{height:min(320px,max(180px,calc(100vh - 580px)));flex:0 0 auto}" in css
    assert ".graph-definition-layout>.cx-panel{min-height:0}" in css
    assert ".graph-definition-layout{height:auto;max-height:none}" in css
    assert ".execution-preview-panel{padding:14px;flex:0 0 auto;min-height:330px;overflow:visible}" in css
    assert ".execution-preview-panel{flex:0 0 auto;min-height:290px}" in css
...
def test_status_and_level_filter_chips_are_bilingual_and_functional():
    ...
    approvals_path = TEMPLATES_DIR / "approvals.html"
...
def test_dynamic_enum_values_use_shared_localized_display_labels():
    ...
    approvals_path = TEMPLATES_DIR / "approvals.html"
    audit_path = TEMPLATES_DIR / "audit.html"
    if approvals_path.exists():
        approvals = approvals_path.read_text(encoding="utf-8")
