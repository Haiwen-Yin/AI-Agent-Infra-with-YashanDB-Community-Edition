...
PROFILE_TEMPLATES = {"graph_stable.html"}
LEGACY_MARKERS = (
...
def validate_product_tree(
    ...
    The source tree intentionally keeps both Graph templates so the build can
    derive the stable and experimental profiles from one line.  A generated
    package contains exactly one ``graph.html`` selected by the profile.
    """
    ...
    graph_profiles = {"production", "graph-preview", "development", "experimental-4.2"}
    if generated and profile in graph_profiles:
        graph_text = (template_dir / "graph.html").read_text(encoding="utf-8") if (template_dir / "graph.html").exists() else ""
        if "Graph Engineering" not in graph_text or "graph-runs" not in graph_text:
            issues.append("graph.html: experimental Graph Engineering controls are missing")
    if generated and profile == "stable-4.1":
        graph_text = (template_dir / "graph.html").read_text(encoding="utf-8") if (template_dir / "graph.html").exists() else ""
        if "Graph Engineering" in graph_text or "graph-runs" in graph_text:
            issues.append("graph.html: stable profile contains experimental Graph controls")
