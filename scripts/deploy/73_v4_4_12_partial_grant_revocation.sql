-- YashanDB rejects a combined REVOKE if any named privilege is absent.
-- Revoke individually so a missing DELETE cannot preserve INSERT/UPDATE.
DECLARE
...
BEGIN
  FOR g IN (SELECT 'DEEP_SEC_SESSION_ROLE' name FROM DUAL UNION ALL SELECT 'AGENT_API' FROM DUAL UNION ALL SELECT 'PUBLIC' FROM DUAL) LOOP
    FOR p IN (SELECT 'INSERT' name FROM DUAL UNION ALL SELECT 'UPDATE' FROM DUAL UNION ALL SELECT 'DELETE' FROM DUAL) LOOP
      revoke_one(p.name,'CX_PRINCIPALS',g.name);
      revoke_one(p.name,'AGENT_REGISTRY',g.name);
    END LOOP;
  END LOOP;
END;
/
