...
def _redact_value(value: Any, fields: Optional[Dict[str, Dict[str, Any]]] = None,
    ...
    if isinstance(value, dict):
        ...
        for child_key, child_value in value.items():
            child_field = (fields or {}).get(str(child_key)) or {}
            result[child_key] = _redact_value(
                child_value, fields, allow_secrets, child_field, str(child_key)
            )
        return result
    if isinstance(value, list):
        return [_redact_value(item, fields, allow_secrets) for item in value]
    return value
