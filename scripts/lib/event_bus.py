"""AI Agent Infra v4.0.0 - Enterprise Edition - Event Bus + Hook Execution
...
def _ensure_dead_letter_table() -> None:
    """Create EVENT_DEAD_LETTER table if it does not exist (idempotent)."""
    create_sql = """
        CREATE TABLE EVENT_DEAD_LETTER (
            DEAD_LETTER_ID     VARCHAR2(64) DEFAULT SYS_GUID() PRIMARY KEY,
            ORIGINAL_EVENT_ID  VARCHAR2(64),
    ...
    try:
        ...
        pass
    try:
        execute("CREATE INDEX IDX_EDL_STATUS ON EVENT_DEAD_LETTER(STATUS)", {})
    except Exception:
        pass
    try:
        execute("CREATE INDEX IDX_EDL_AGENT ON EVENT_DEAD_LETTER(AGENT_ID)", {})
    except Exception:
        pass
...
def send_to_dead_letter(
    ...
    try:
        ...
        insert_sql = """
            ...
            VALUES (RAWTOHEX(SYS_GUID()), :orig, :aid, :etype,
                    :payload, :reason, 'PENDING', SYSTIMESTAMP)
