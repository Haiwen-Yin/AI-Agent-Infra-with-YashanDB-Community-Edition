...
class ReplayGuard:
    """Bounded process-local nonce guard for signed event admission.

    The Inbox idempotency key remains the durable duplicate barrier.  This
    guard closes the shorter replay window before a duplicate reaches the
    database and deliberately evicts old entries so an event flood cannot
    grow process memory without a bound.
    """

    def __init__(self, *, max_entries: int = 10000, ttl_seconds: int = DEFAULT_REPLAY_WINDOW_SECONDS):
        self.max_entries = max(1, min(int(max_entries), 1000000))
        self.ttl_seconds = max(1, min(int(ttl_seconds), 86400))
        self._seen: OrderedDict[tuple[str, str, str], float] = OrderedDict()

    def check_and_remember(self, subject: str, key_id: str, nonce: str, *, now: Optional[float] = None) -> bool:
        ...
        marker = (str(subject), str(key_id), str(nonce))
        if marker in self._seen:
            self._seen.move_to_end(marker)
            return False
        self._seen[marker] = current
...
def verify_event_auth(source_ref: str, event_type: str, schema_version: str,
                      ...
                      *, now: Optional[float] = None,
                      ...
                      replay_guard: Optional[ReplayGuard] = None) -> Dict[str, Any]:
    ...
    if replay_guard is not None and not errors:
        if not replay_guard.check_and_remember(subject, key_id, nonce, now=current):
            errors.append({"code": "EVENT_NONCE_REPLAY"})
