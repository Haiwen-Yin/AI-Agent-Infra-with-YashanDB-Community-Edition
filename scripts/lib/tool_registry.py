"""AI Agent Infra v4.0.1 - Enterprise Edition - Tool Registry + DAG Chains
    ...
    for order, step in enumerate(steps, 1):
        execute_insert(
            """INSERT INTO TOOL_CHAIN_STEPS
               ...
               VALUES (AI_NEW_ID(), :cid, :p_order, :tid,
...
def record_tool_call(tool_id: str) -> bool:
    affected = execute(
        """UPDATE TOOL_REGISTRY SET CALL_COUNT = CALL_COUNT + 1, LAST_CALLED_AT = CURRENT_TIMESTAMP
           WHERE TOOL_ID = :tid""",
        {"tid": tool_id},
    )
    return affected > 0
