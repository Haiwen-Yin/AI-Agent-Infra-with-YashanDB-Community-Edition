"""AI Agent Infra v4.0.0 - Community Edition - Embedding API
...
def _ensure_embedding_version_column() -> bool:
    """Best-effort ALTER TABLE to add EMBEDDING_VERSION if missing."""
    try:
        from .connection import get_connection
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "ALTER TABLE ENTITY_EMBEDDINGS ADD (EMBEDDING_VERSION NUMBER DEFAULT 1)"
                )
            conn.commit()
        return True
    except Exception as e:
        # Most likely ORA-01430 (column already exists) - safe to ignore
        logger.debug(f"EMBEDDING_VERSION column add skipped: {e}")
        return False
        ...
        sql = f"""
            ...
            WHERE e.TITLE IS NOT NULL
              ...
              AND (
                  ...
                  OR NVL(em.EMBEDDING_VERSION, 1) < :tv
              )
            ORDER BY e.CREATED_AT DESC
