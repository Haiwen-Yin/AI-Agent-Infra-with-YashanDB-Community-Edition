"""AI Agent Infra v4.0.0 - Enterprise Edition - Monitoring & Observability
...
def get_performance_metrics(since: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        avg_session = execute_query_one(
            f"SELECT AVG(EXTRACT(EPOCH FROM (COMPLETED_AT - CREATED_AT))) AS VAL "
            f"FROM AGENT_SESSION WHERE IS_ACTIVE = 'N' "
            f"AND LAST_ACTIVE_AT > {_pg_interval(7 * 86400)}"
        )
    except Exception:
        try:
            avg_session = execute_query_one(
                ...
                "AND START_TIME > SYSTIMESTAMP - NUMTODSINTERVAL(7, 'DAY')"
            )
        except Exception as e:
    ...
    try:
        ...
        try:
            avg_loop = execute_query_one(
                ...
                "AND STARTED_AT > SYSTIMESTAMP - NUMTODSINTERVAL(7, 'DAY')"
            )
        except Exception as e:
    ...
    try:
        avg_iterations = execute_query_one(
            f"SELECT AVG(ITERATION_COUNT) AS VAL FROM LOOP_RUNS "
            f"WHERE STATUS = 'COMPLETED' AND STARTED_AT > {_pg_interval(7 * 86400)}"
        )
    except Exception:
        try:
            avg_iterations = execute_query_one(
                "SELECT AVG(ITERATION_COUNT) AS VAL FROM LOOP_RUNS "
                "WHERE STATUS = 'COMPLETED' AND STARTED_AT > SYSTIMESTAMP - NUMTODSINTERVAL(7, 'DAY')"
            )
        except Exception as e:
    ...
    try:
        avg_tool = execute_query_one(
            f"SELECT AVG(DURATION_MS) AS VAL FROM TASK_TOOL_CALLS "
            f"WHERE CREATED_AT > {_pg_interval(7 * 86400)}"
        )
    except Exception:
        try:
            avg_tool = execute_query_one(
                "SELECT AVG(DURATION_MS) AS VAL FROM TASK_TOOL_CALLS "
                "WHERE CREATED_AT > SYSTIMESTAMP - NUMTODSINTERVAL(7, 'DAY')"
            )
        except Exception as e:
...
def _ensure_alert_rules_table() -> bool:
    """Best-effort CREATE TABLE for ALERT_RULES if missing."""
    try:
        ...
        logger.debug(f"ALERT_RULES table creation skipped: {e}")
        return False
        ...
        try:
            ...
            stale_rows = execute_query(
                f"""SELECT AGENT_ID, AGENT_NAME, STATUS, LAST_SEEN_AT, LAST_ACTIVE_AT
                    FROM AGENT_REGISTRY
                    WHERE LAST_SEEN_AT IS NOT NULL
                      AND LAST_SEEN_AT < SYSTIMESTAMP - INTERVAL '{int(stale_threshold_minutes)}' MINUTE
                    ORDER BY LAST_SEEN_AT ASC""",
                {},
            )
    except Exception as e:
...
def evaluate_alert_rules() -> List[Dict[str, Any]]:
    ...
    for trigger in snapshot.get("actions_pending", []):
        ...
        try:
            execute(
                """UPDATE ALERT_RULES
                       SET LAST_TRIGGERED_AT = SYSTIMESTAMP
                   WHERE RULE_ID = :rid""",
