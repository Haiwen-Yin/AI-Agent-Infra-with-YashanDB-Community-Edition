...
def _is_supported_python_command(executable: str) -> bool:
    """Allow an explicitly versioned Python 3.14+ command by default."""
    match = re.fullmatch(r"python3\.(\d+)", executable)
...
def _run_command(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if executable not in allowed and not (
        allowed == DEFAULT_ALLOWED_COMMANDS and _is_supported_python_command(executable)
    ):
