"""AI Agent Infra v4.0.1 - Enterprise Edition - Skill Acquisition API
...
def discover_skills(
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).debug(f"discover_skills Oracle-schema query failed: {e}")
    where_parts = ["sm.STATUS = 'ACTIVE'"]
    pg_params: Dict[str, Any] = {"lim": 50, "off": 0}
    if skill_type:
        where_parts.append("sm.SKILL_TYPE = :vstype")
        pg_params["vstype"] = skill_type
    if keyword:
        where_parts.append("UPPER(sm.SKILL_NAME) LIKE UPPER(:vkw)")
        pg_params["vkw"] = f"%{keyword}%"
    sql_pg = f"""
        SELECT sm.SKILL_ID AS ENTITY_ID, sm.SKILL_NAME AS TITLE, sm.CATEGORY, sm.STATUS,
               sm.VISIBILITY, sm.OWNED_BY_AGENT,
               sm.SKILL_NAME, sm.SKILL_VERSION, sm.SKILL_TYPE,
               CAST(NULL AS VARCHAR) AS SKILL_FORMAT,
               CAST(NULL AS VARCHAR) AS RUNTIME,
               sm.STATUS AS SKILL_STATUS, sm.DESCRIPTION AS SKILL_DESCRIPTION,
    ...
    """
    try:
        return execute_query(sql_pg, pg_params)
    except Exception as e:
        import logging
        logging.getLogger(__name__).debug(f"discover_skills PG fallback failed: {e}")
        return []
...
def acquire_skill_text(skill_id: str) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "package_checksum": skill.get("resource_checksum"),
    }
...
def materialize_skill(skill_id: str, install_root: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if metadata is None or archive_bytes is None:
        return None
    expected_hash = metadata.get("package_checksum")
    root = Path(install_root or os.environ.get(
        "AI_AGENT_SKILL_ROOT", str(Path.home() / ".ai-agent-infra" / "skills")
    )).expanduser()
    safe_name = "".join(ch for ch in str(metadata.get("skill_name") or skill_id)
                        if ch.isalnum() or ch in "_-")
    safe_version = "".join(ch for ch in str(metadata.get("skill_version") or "1.0.0")
                           if ch.isalnum() or ch in ".-_")
    target = root / safe_name / safe_version
    staging_parent = root / ".staging"
    staging_parent.mkdir(parents=True, exist_ok=True)
    staging = Path(tempfile.mkdtemp(prefix=f"{safe_name}-", dir=staging_parent))
        ...
        with zipfile.ZipFile(io.BytesIO(archive_bytes)) as archive:
            for info in sorted(archive.infolist(), key=lambda item: item.filename):
                ...
                if (info.filename.startswith("/") or "\\" in info.filename
                    ...
                    raise ValueError(f"Unsafe acquired Skill path: {info.filename}")
                content = archive.read(info)
                digest = hashlib.sha256(content).hexdigest()
                file_hashes[path.as_posix()] = digest
                package_hash.update(path.as_posix().encode("utf-8") + b"\0")
                package_hash.update(bytes.fromhex(digest))
                destination = staging.joinpath(*path.parts)
                destination.parent.mkdir(parents=True, exist_ok=True)
                destination.write_bytes(content)
        actual_hash = package_hash.hexdigest()
        if expected_hash and actual_hash != expected_hash:
            raise ValueError("Acquired Skill package checksum mismatch")
        status = "INSTALLED"
        if target.exists():
            installed_hashes = {
            ...
            if installed_hashes != file_hashes:
                raise ValueError("Installed Skill tree does not match acquired package")
