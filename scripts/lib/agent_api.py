        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if int(cur.fetchone()[0]) == 0:
                    ...
                    cur.execute(f'ALTER USER "{username}" IDENTIFIED BY "{password}"')
                cur.execute(f'GRANT DEEP_SEC_SESSION_ROLE TO "{username}"')
                conn.commit()
        execute("""
            MERGE INTO SYSTEM_CONFIG sc
            USING (SELECT :config_key AS k, :config_value AS v FROM DUAL) src
            ...
            WHEN NOT MATCHED THEN INSERT (CONFIG_KEY, CONFIG_VALUE, DESCRIPTION)
                VALUES (src.k, src.v, :description)
        """, {
            "config_key": f"end_user_pwd.{agent_id}",
    ...
    for row in assigned:
        for session in get_active_sessions(row.get("agent_id")):
            try:
                ...
                _logger.warning("Unable to close stale Portal session for %s", row.get("agent_id"))
    return execute("""
        UPDATE AGENT_REGISTRY
        SET STATUS = 'POOL', CURRENT_USER_ID = NULL, PORTAL_NODE_ID = NULL,
            UPDATED_AT = SYSTIMESTAMP
...
def recover_agent_via_admin(
    ...
    try:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(f'ALTER USER "{eu_name}" IDENTIFIED BY "{new_pwd}"')
                conn.commit()
    except Exception as e:
