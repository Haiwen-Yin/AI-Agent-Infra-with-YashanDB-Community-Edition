...
def _is_unique_violation(exc: Exception) -> bool:
    """Identify a duplicate-key race without masking unrelated DB failures."""
...
def _is_missing_governance_event_table(exc: Exception) -> bool:
    message = str(exc).lower()
    return any(fragment in message for fragment in (
...
def _record_governance_event(event_type: str, actor_id: str, reason: str, *,
                             run_id: Optional[str] = None, artifact_id: Optional[str] = None,
                             detail: Optional[Dict[str, Any]] = None,
                             tx: Any = None) -> Optional[str]:
    """Use the single governance event contract for standalone and tx-bound writes."""
    from . import graph_governance
    return graph_governance.record_governance_event(
        event_type, actor_id, reason, run_id=run_id, artifact_id=artifact_id,
        detail=detail, tx=tx,
    )
...
def _lease_is_expired(expires: Any) -> bool:
...
def _executor_manifests_tx(tx) -> List[Dict[str, Any]]:
    """Load the active registry without hiding unexpected database errors."""
    manifests = graph_executor.builtin_executor_manifests()
    try:
        rows = tx.query(
        ...
        if graph_executor._is_registry_missing_error(exc):
            return manifests
        raise
    manifests.extend(graph_executor.manifests_from_rows(rows))
    return manifests
...
def _intervention_tx(tx, run_id: str, action_name: str, actor_id: str, reason: str,
    ...
    _record_governance_event(
        f"RUN_{str(action_name or 'CONTROL').upper()[:48]}", actor_id, reason,
        run_id=run_id, detail={"node_run_id": node_run_id, "evidence": evidence or {}}, tx=tx,
    )
...
def _verify_lease(token: str, operation: str) -> Optional[Dict[str, Any]]:
    ...
    if operation not in operations:
        return None
    if _lease_is_expired(lease.get("expires_at")):
        return None
    return lease


def _verify_completion_lease(token: str) -> Optional[Dict[str, Any]]:
    """Verify a completion token, retaining only exact terminal replay access."""
    if not token:
        return None
    token_hash = hashlib.sha256(token.encode("utf-8")).hexdigest()
    lease = _row(connection.execute_query_one(
        "SELECT l.LEASE_ID, l.ATTEMPT_ID, l.WORKER_ID, l.TOKEN_HASH, l.FENCING_TOKEN, "
        "l.OPERATIONS_JSON, l.SCOPE_JSON, l.EXPIRES_AT, l.REVOKED_AT, "
        "a.RUN_ID, a.NODE_RUN_ID, a.STATUS, a.FENCING_TOKEN AS ATTEMPT_FENCING_TOKEN, "
        "a.COMPLETION_DIGEST, nr.NODE_KEY FROM GRAPH_LEASE_TOKENS l "
        "JOIN GRAPH_ATTEMPTS a ON a.ATTEMPT_ID = l.ATTEMPT_ID "
        "JOIN GRAPH_NODE_RUNS nr ON nr.NODE_RUN_ID = a.NODE_RUN_ID "
        "WHERE l.TOKEN_HASH = :token_hash AND a.FENCING_TOKEN = l.FENCING_TOKEN",
        {"token_hash": token_hash},
    ))
    if not lease or "complete" not in (lease.get("operations") or []):
        return None
    status = str(lease.get("status") or "").upper()
    if status == "RUNNING":
        if lease.get("revoked_at") or _lease_is_expired(lease.get("expires_at")):
            return None
...
def _completion_request_digest(output_state: Optional[Dict[str, Any]], evidence: Optional[Dict[str, Any]]) -> str:
    """Hash the full worker request without persisting the request contents."""
    return completion_request_digest(output_state, evidence)


def _existing_completion_result_tx(tx, attempt: Dict[str, Any], request_digest: str) -> Dict[str, Any]:
    """Return the original commit result for an exact completion replay."""
    if str(attempt.get("completion_digest") or "") != request_digest:
        raise ValueError("completion payload conflicts with the committed result")
    node_run = tx.query_one(
        "SELECT OUTPUT_CHECKPOINT_ID FROM GRAPH_NODE_RUNS WHERE NODE_RUN_ID = :node_run_id",
        {"node_run_id": attempt["node_run_id"]},
    )
    checkpoint_id = (node_run or {}).get("output_checkpoint_id")
    checkpoint = _row(tx.query_one(
        "SELECT CHECKPOINT_ID, SEQ_NO, STATE_JSON, STATE_HASH FROM GRAPH_CHECKPOINTS "
        "WHERE CHECKPOINT_ID = :checkpoint_id AND RUN_ID = :run_id",
        {"checkpoint_id": checkpoint_id, "run_id": attempt["run_id"]},
    ))
    if not checkpoint:
        raise RuntimeError("committed completion checkpoint is missing")
    event = _row(tx.query_one(
        "SELECT EVENT_ID FROM GRAPH_STATE_EVENTS WHERE CHECKPOINT_ID = :checkpoint_id "
        "AND RUN_ID = :run_id",
        {"checkpoint_id": checkpoint_id, "run_id": attempt["run_id"]},
    ))
    run = tx.query_one(
        "SELECT STATUS FROM GRAPH_RUNS WHERE RUN_ID = :run_id", {"run_id": attempt["run_id"]},
    )
    run_status = str((run or {}).get("status") or "SUCCEEDED").upper()
    return {
        "attempt_id": attempt["attempt_id"],
        "checkpoint": {
            "checkpoint_id": checkpoint.get("checkpoint_id"),
            "event_id": (event or {}).get("event_id"),
            "seq_no": checkpoint.get("seq_no"),
            "state": _redact(checkpoint.get("state") or {}),
            "state_hash": checkpoint.get("state_hash"),
        },
        "status": "REVIEW_REQUIRED" if run_status == "REVIEW_REQUIRED" else "SUCCEEDED",
...
def claim_ready(worker_id: str, runtime: str, capabilities: Optional[List[str]] = None,
    ...
    def _claim(tx):
        ...
        for candidate in candidates:
            ...
            if deadline is not None:
                ...
                if isinstance(parsed_deadline, datetime) and parsed_deadline <= datetime.now(timezone.utc).replace(tzinfo=None):
                    ...
                    continue
            policy = tx.query_one(
                "SELECT r.GRAPH_VERSION_ID, r.ACTOR_ID, n.NODE_TYPE, n.TYPE_VERSION, n.RESOURCE_SCOPE_JSON, n.BUDGET_JSON AS NODE_BUDGET_JSON, "
            ...
            if policy:
                ...
                executor_manifests = _executor_manifests_tx(tx)
                try:
                    executor_admission = graph_executor.admission({
                        "node_type": policy.get("node_type"),
                        "type_version": policy.get("type_version") or "1.0",
                        "config": node_config,
                        "side_effect_class": policy.get("side_effect_class") or "NONE",
                    }, manifests=executor_manifests)
                    ...
                    tx.execute(
                        "UPDATE GRAPH_RUNS SET STATUS = 'REVIEW_REQUIRED', ERROR_CODE = 'EXECUTOR_UNAVAILABLE', "
                        "ERROR_MESSAGE = :error_message, UPDATED_AT = CURRENT_TIMESTAMP WHERE RUN_ID = :run_id "
                        "AND STATUS = 'RUNNING'",
                        {"run_id": candidate["run_id"], "error_message": str(exc)[:2000]},
                    )
                    _trace_tx(tx, candidate["run_id"], "EXECUTOR_REJECTED", status="REVIEW_REQUIRED",
                              detail={"node_key": candidate["node_key"], "error": str(exc)})
                    continue
                ...
                ready["executor_admission"] = executor_admission
                ready["effect_idempotency_key"] = graph_executor.effect_idempotency_key(
                    ready["run_id"], ready["node_run_id"],
                    str(policy.get("side_effect_class") or "NONE").upper(),
                )
                break
        if not ready:
            return None
        ...
        tx.execute(
            ...
            "FENCING_TOKEN, LEASE_EXPIRES_AT, INPUT_STATE_JSON, IDEMPOTENCY_KEY, EFFECT_IDEMPOTENCY_KEY, STARTED_AT) VALUES "
            "(:attempt_id, :node_run_id, :run_id, :worker_id, 'RUNNING', :fencing_token, "
            ":lease_expires_at, :input_state_json, :idempotency_key, :effect_idempotency_key, CURRENT_TIMESTAMP)",
            {"attempt_id": attempt_id, "node_run_id": ready["node_run_id"], "run_id": ready["run_id"],
             ...
             "effect_idempotency_key": ready.get("effect_idempotency_key")},
        )
        if tx.execute(
        ...
        lease = _issue_lease_tx(
            tx, attempt_id, worker_id, fencing,
            {
                ...
                "executor_admission": ready.get("executor_admission"),
                "effect_idempotency_key": ready.get("effect_idempotency_key"),
        ...
        _record_governance_event(
            "NODE_CLAIMED", str(ready.get("worker_agent_id") or worker_id),
            "Graph Node claimed", run_id=ready["run_id"],
            detail={"node_run_id": ready["node_run_id"], "node_key": ready["node_key"],
...
def _insert_transition_tx(tx, *, run_id: str, node_run_id: str, attempt_id: Optional[str],
                          from_node_key: str, to_node_key: str, edge_id: Optional[str],
                          checkpoint_id: str, evidence: Dict[str, Any], actor_id: str) -> None:
    """Insert one logical Edge transition idempotently across all adapters."""
    if attempt_id:
        existing = tx.query_one(
            "SELECT TRANSITION_ID FROM GRAPH_TRANSITIONS WHERE ATTEMPT_ID = :attempt_id",
            {"attempt_id": attempt_id},
        )
        if existing:
            return
    transition_id = _hash({
        "run_id": run_id, "node_run_id": node_run_id, "attempt_id": attempt_id,
        "edge_id": edge_id, "checkpoint_id": checkpoint_id,
    })[:128]
    params = {
        "transition_id": transition_id, "run_id": run_id, "node_run_id": node_run_id,
        "attempt_id": attempt_id, "from_node_key": from_node_key, "to_node_key": to_node_key,
        "edge_id": edge_id, "checkpoint_id": checkpoint_id, "fencing_token": None,
        "evidence_json": _canonical(_redact(evidence or {})), "actor_id": actor_id,
    }
    dialect = str(getattr(connection, "DATABASE_DIALECT", "") or "").lower()
    if dialect == "postgresql":
        tx.execute(
            ...
            "ON CONFLICT (TRANSITION_ID) DO NOTHING", params,
        )
        return
    tx.execute(
        "MERGE INTO GRAPH_TRANSITIONS dst USING (SELECT :transition_id AS TRANSITION_ID"
        + connection.merge_scalar_suffix() + ") src ON (dst.TRANSITION_ID = src.TRANSITION_ID) "
        "WHEN NOT MATCHED THEN INSERT (TRANSITION_ID, RUN_ID, NODE_RUN_ID, ATTEMPT_ID, FROM_NODE_KEY, "
        "TO_NODE_KEY, EDGE_ID, TRANSITION_TYPE, STATUS, CHECKPOINT_ID, FENCING_TOKEN, EVIDENCE_JSON, "
        "ACTOR_ID, CREATED_AT) VALUES (:transition_id, :run_id, :node_run_id, :attempt_id, "
        ":from_node_key, :to_node_key, :edge_id, 'EDGE', 'COMMITTED', :checkpoint_id, "
        ":fencing_token, :evidence_json, :actor_id, CURRENT_TIMESTAMP)", params,
    )
...
def _activate_downstream_tx(tx, run_id: str, node_run_id: str, checkpoint_id: str,
    ...
    for edge_index, edge in enumerate(selected):
        ...
        _insert_transition_tx(
            tx, run_id=run_id, node_run_id=node_run_id,
            attempt_id=attempt_id if edge_index == 0 else None,
            from_node_key=node["node_key"], to_node_key=target,
            edge_id=edge.get("edge_id"), checkpoint_id=checkpoint_id,
            evidence=evidence or {}, actor_id=actor_id,
        )
...
def _evaluate_node_tx(tx, run_id: str, node_run_id: str, node_config: Dict[str, Any],
                      safe_output: Dict[str, Any], evidence: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """Persist a declared Loop/Graph evaluator observation in the same commit."""
    config = node_config.get("evaluation_config") or node_config.get("evaluator_config")
    if not config and node_config.get("evaluator"):
        config = {"evaluator": node_config.get("evaluator")}
    if not config:
        return None
    if isinstance(config, str):
        config = {"evaluator": config}
    if not isinstance(config, dict):
        raise ValueError("evaluation_config must be an object or evaluator name")
    from . import graph_evaluators
    contract = graph_evaluators.normalize_config(config)
    observation = dict(evidence or {})
    observation.setdefault("output", safe_output)
    result = graph_evaluators.evaluate_observation(config, observation)
    evaluation_id = _id("EVAL")
    tx.execute(
        "INSERT INTO GRAPH_EVALUATIONS (EVALUATION_ID, RUN_ID, NODE_RUN_ID, EVALUATOR_NAME, "
        "EVALUATOR_VERSION, LEVEL_NAME, INPUT_JSON, RESULT_JSON, ROUTE_DECISION, CREATED_AT) "
        "VALUES (:evaluation_id, :run_id, :node_run_id, :evaluator_name, :evaluator_version, "
        ":level_name, :input_json, :result_json, :route_decision, CURRENT_TIMESTAMP)",
        {"evaluation_id": evaluation_id, "run_id": run_id, "node_run_id": node_run_id,
         "evaluator_name": contract["type_name"], "evaluator_version": contract["type_version"],
         "level_name": contract["level"], "input_json": _canonical(_redact(observation)),
         "result_json": _canonical(_redact(result)), "route_decision": result.get("route_decision")},
    )
...
def complete_attempt(lease_token: str, output_state: Optional[Dict[str, Any]], actor_id: str,
    ...
    request_digest = _completion_request_digest(output_state, evidence)
    lease = _verify_completion_lease(lease_token)
    if not lease:
        raise PermissionError("invalid, expired, or revoked lease token")
    def _complete(tx):
        attempt = tx.query_one(
            "SELECT ATTEMPT_ID, NODE_RUN_ID, RUN_ID, FENCING_TOKEN, STATUS, WORKER_ID, "
            "COMPLETION_DIGEST FROM GRAPH_ATTEMPTS WHERE ATTEMPT_ID = :attempt_id FOR UPDATE",
            {"attempt_id": lease["attempt_id"]},
        )
        if not attempt:
            raise RuntimeError("Graph Attempt not found")
        if str(attempt.get("status") or "").upper() == "SUCCEEDED":
            return _existing_completion_result_tx(tx, attempt, request_digest)
        if str(attempt.get("status") or "").upper() != "RUNNING" or int(attempt.get("fencing_token") or 0) != int(lease.get("fencing_token") or 0):
            raise RuntimeError("stale attempt fencing token")
        locked_lease = _row(tx.query_one(
            "SELECT LEASE_ID, ATTEMPT_ID, TOKEN_HASH, FENCING_TOKEN, OPERATIONS_JSON, "
            "EXPIRES_AT, REVOKED_AT FROM GRAPH_LEASE_TOKENS "
            "WHERE LEASE_ID = :lease_id AND TOKEN_HASH = :token_hash FOR UPDATE",
            {"lease_id": lease["lease_id"], "token_hash": lease["token_hash"]},
        ))
        if (
            not locked_lease
            or locked_lease.get("revoked_at")
            or "complete" not in (locked_lease.get("operations") or [])
            or int(locked_lease.get("fencing_token") or 0) != int(attempt.get("fencing_token") or 0)
            or _lease_is_expired(locked_lease.get("expires_at"))
        ):
            raise PermissionError("invalid, expired, or revoked lease token")
        node_policy = tx.query_one(
            "SELECT n.NODE_TYPE, n.TYPE_VERSION, n.OUTPUT_SCHEMA_JSON, n.CONFIG_JSON, n.RESOURCE_SCOPE_JSON, n.SIDE_EFFECT_CLASS "
            "FROM GRAPH_NODE_RUNS nr JOIN GRAPH_RUNS r ON r.RUN_ID = nr.RUN_ID "
            "JOIN GRAPH_NODES n ON n.GRAPH_VERSION_ID = r.GRAPH_VERSION_ID AND n.NODE_KEY = nr.NODE_KEY "
        ...
        executor_manifests = _executor_manifests_tx(tx)
        executor_admission = graph_executor.admission({
            "node_type": node_policy.get("node_type"),
            "type_version": node_policy.get("type_version") or "1.0",
            "config": node_config,
            "side_effect_class": node_policy.get("side_effect_class") or "NONE",
        }, manifests=executor_manifests)
        ...
        evaluation = _evaluate_node_tx(
            tx, attempt["run_id"], attempt["node_run_id"], node_config, safe_output, evidence,
        )
        ...
        if tx.execute(
            ...
            {"attempt_id": attempt["attempt_id"], "output_state_json": _canonical(safe_output),
             "completion_digest": request_digest, "fencing_token": lease["fencing_token"]},
        ) != 1:
        ...
        _trace_tx(
            ...
            estimated_cost=(evidence or {}).get("estimated_cost"),
            detail={**(evidence or {}), "executor_admission": executor_admission, "evaluation": evaluation},
        )
        _record_governance_event(
            "NODE_COMPLETED", actor_id, "Graph Node completed", run_id=attempt["run_id"],
            detail={"node_run_id": attempt["node_run_id"], "attempt_id": attempt["attempt_id"],
                    "status": "SUCCEEDED", "evaluation_id": (evaluation or {}).get("evaluation_id")}, tx=tx,
        )
        tx.execute(
...
def fail_attempt(lease_token: str, error_code: str, error_message: str, actor_id: str) -> bool:
    ...
    def _fail(tx):
        ...
        if attempt:
            ...
            retry_decision = graph_executor.side_effect_retry_decision(
                side_effect, error_code, attempts_used, max_attempts,
            )
            uncertain = bool(retry_decision["uncertain_outcome"])
            if retry_decision["retry"]:
                ...
                terminal_status = retry_decision["status"]
                tx.execute(
                ...
                _trace_tx(tx, attempt["run_id"], "ATTEMPT_FAILED", node_run_id=attempt["node_run_id"],
                          ...
                          detail={"error_code": error_code, "error_message": str(error_message or "")[:500],
                                  ...
                                  "automatic_retry": retry_decision["automatic_retry"],
                                  "retry_reason": retry_decision["reason"]})
                _record_governance_event(
                    "NODE_FAILED", actor_id, "Graph Node failed", run_id=attempt["run_id"],
                    detail={"node_run_id": attempt["node_run_id"], "attempt_id": attempt["attempt_id"],
                            "error_code": error_code, "uncertain_outcome": uncertain,
                            "automatic_retry": retry_decision["automatic_retry"],
                            "retry_reason": retry_decision["reason"]}, tx=tx,
                )
        tx.execute(
            "UPDATE GRAPH_LEASE_TOKENS SET REVOKED_AT = CURRENT_TIMESTAMP WHERE ATTEMPT_ID = :attempt_id "
            "AND FENCING_TOKEN = :fencing_token AND REVOKED_AT IS NULL",
...
def list_attempts(run_id: str, node_run_id: Optional[str] = None, limit: int = 500) -> List[Dict[str, Any]]:
    ...
    return _rows(connection.execute_query(
        "SELECT ATTEMPT_ID, NODE_RUN_ID, RUN_ID, WORKER_ID, STATUS, FENCING_TOKEN, LEASE_EXPIRES_AT, INPUT_STATE_JSON, OUTPUT_STATE_JSON, RESULT_ARTIFACT_ID, IDEMPOTENCY_KEY, EFFECT_IDEMPOTENCY_KEY, ERROR_CODE, ERROR_MESSAGE, STARTED_AT, COMPLETED_AT "
...
def get_evaluation_metrics(run_id: str) -> Dict[str, Any]:
    """Compute the persisted Graph run metrics without running evaluators."""
    from . import graph_evaluators
    run = get_run(run_id) or {}
    plan = _plan_for_run(run_id)
    transitions = list_transitions(run_id, limit=5000)
    attempts = list_attempts(run_id, limit=5000)
    interventions = list_interventions(run_id, limit=5000)
    budget = run.get("budget") or {}
    usage = run.get("budget_usage") or {}
    planned_nodes = len(plan.get("nodes") or []) if isinstance(plan, dict) else None
    return graph_evaluators.compute_metrics(
        transitions=transitions, attempts=attempts, interventions=interventions,
        budget=budget, usage=usage, planned_nodes=planned_nodes,
    )


def list_governance_events(run_id: Optional[str] = None, artifact_id: Optional[str] = None,
    ...
    if artifact_id:
        conditions.append("ARTIFACT_ID = :artifact_id")
    ...
    try:
        rows = connection.execute_query(
            "SELECT EVENT_ID, RUN_ID, ARTIFACT_ID, EVENT_TYPE, ACTOR_ID, REASON, DETAIL_JSON, CREATED_AT "
            "FROM GRAPH_GOVERNANCE_EVENTS WHERE " + " AND ".join(conditions) +
            " ORDER BY CREATED_AT DESC FETCH FIRST :limit ROWS ONLY", params,
        )
    except Exception as exc:
        if _is_missing_governance_event_table(exc):
            return []
        raise
    return _rows(rows)
...
def export_run_evidence(run_id: str, actor_id: str, reason: str, limit: int = 500) -> Dict[str, Any]:
    """Export a bounded, redacted Run evidence document with a stable digest."""
    if not str(actor_id or "").strip() or not str(reason or "").strip():
        raise ValueError("Run evidence export requires actor_id and reason")
    snapshot = get_run_snapshot(run_id, limit=_limit(limit, default=500, maximum=1000))
    if not snapshot:
        raise ValueError("Graph Run not found")
    document = {
        "schema": "ai-agent-infra-graph-run-evidence/v1",
        "run_id": run_id,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "snapshot": _redact(snapshot),
    }
    evidence_hash = _hash(document)
    governance_event_id = _record_governance_event(
        "RUN_EVIDENCE_EXPORTED", actor_id, reason, run_id=run_id,
        detail={"evidence_hash": evidence_hash, "record_limit": _limit(limit, default=500, maximum=1000)},
    )
    document["evidence_hash"] = evidence_hash
    document["governance_event_id"] = governance_event_id
    return document
...
def ingest_event(source_ref: str, event_type: str, schema_version: str, idempotency_key: str,
    ...
    try:
        connection.execute(
            ...
            {"inbox_id": inbox_id, "source_ref": source_ref, "event_type": str(event_type).upper(),
             ...
             "status": "DEAD_LETTER" if auth_result["poison"] else "RECEIVED",
    ...
    if auth_result["activation_allowed"]:
        ...
        try:
            ...
            # Persist a deterministic activation failure for operator replay;
            # do not acknowledge the event as successfully processed.
            attempt_row = connection.execute_query_one(
                "SELECT ATTEMPTS FROM GRAPH_INBOX WHERE INBOX_ID = :inbox_id",
                {"inbox_id": inbox_id},
            ) or {}
            attempts = int(attempt_row.get("attempts") or 1)
            if attempts >= 5:
                status = "DEAD_LETTER"
                available_at = None
            else:
                status = "RECEIVED"
                available_at = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(
                    seconds=min(3600, 5 * (2 ** min(attempts - 1, 20)))
                )
            connection.execute(
                "UPDATE GRAPH_INBOX SET STATUS = :status, ERROR_MESSAGE = :error_message, "
                "AVAILABLE_AT = COALESCE(:available_at, AVAILABLE_AT), PROCESSED_AT = "
                "CASE WHEN :status = 'DEAD_LETTER' THEN CURRENT_TIMESTAMP ELSE NULL END "
                "WHERE INBOX_ID = :inbox_id AND STATUS = 'PROCESSING'",
                {"inbox_id": inbox_id, "status": status, "available_at": available_at,
                 "error_message": str(exc)[:2000]},
            )
            return {"inbox_id": inbox_id, "duplicate": False, "status": status,
                    "activation_blocked": status == "DEAD_LETTER", "poison": status == "DEAD_LETTER",
                    "retry_scheduled": status == "RECEIVED",
                    "errors": [{"code": "EVENT_ACTIVATION_FAILED", "message": str(exc)[:500]}],
                    "waits_resolved": 0}
        connection.execute(
            "UPDATE GRAPH_INBOX SET STATUS = 'PROCESSED', PROCESSED_AT = CURRENT_TIMESTAMP "
            "WHERE INBOX_ID = :inbox_id AND STATUS = 'PROCESSING'", {"inbox_id": inbox_id},
        )
...
def replay_dead_letter(inbox_id: str, actor_id: str, reason: str) -> Dict[str, Any]:
    """Replay a dead-letter event only through an explicit operator action."""
    if not str(inbox_id or "").strip() or not str(actor_id or "").strip() or not str(reason or "").strip():
        raise ValueError("dead-letter replay requires inbox_id, actor_id, and reason")
    row = connection.execute_query_one(
        "SELECT INBOX_ID, SOURCE_REF, EVENT_TYPE, SCHEMA_VERSION, IDEMPOTENCY_KEY, PAYLOAD_JSON, STATUS "
        "FROM GRAPH_INBOX WHERE INBOX_ID = :inbox_id", {"inbox_id": inbox_id},
    )
    if not row:
        return {"success": False, "inbox_id": inbox_id, "status": "NOT_FOUND"}
    if str(row.get("status") or "").upper() != "DEAD_LETTER":
        raise ValueError("only DEAD_LETTER events can be replayed")
    claimed = connection.execute(
        "UPDATE GRAPH_INBOX SET STATUS = 'PROCESSING', ERROR_MESSAGE = NULL, PROCESSED_AT = NULL "
        "WHERE INBOX_ID = :inbox_id AND STATUS = 'DEAD_LETTER'",
        {"inbox_id": inbox_id},
    )
    if claimed != 1:
        return {"success": False, "inbox_id": inbox_id, "status": "REPLAY_IN_PROGRESS"}
    payload = _json(row.get("payload_json"), {}) or {}
    try:
        resolved = resolve_waits(
            str(row.get("event_type") or "").upper(), payload,
            payload.get("correlation_key") if isinstance(payload, dict) else None,
        )
    except Exception as exc:
        connection.execute(
            "UPDATE GRAPH_INBOX SET STATUS = 'DEAD_LETTER', ERROR_MESSAGE = :error_message, PROCESSED_AT = NULL "
            "WHERE INBOX_ID = :inbox_id AND STATUS = 'PROCESSING'",
            {"inbox_id": inbox_id, "error_message": str(exc)[:2000]},
        )
        _record_governance_event(
            "EVENT_REPLAY_FAILED", actor_id, reason, detail={"inbox_id": inbox_id, "error": str(exc)[:500]},
        )
        raise
    connection.execute(
        "UPDATE GRAPH_INBOX SET STATUS = 'PROCESSED', ERROR_MESSAGE = NULL, PROCESSED_AT = CURRENT_TIMESTAMP "
        "WHERE INBOX_ID = :inbox_id AND STATUS = 'PROCESSING'", {"inbox_id": inbox_id},
    )
    _record_governance_event(
        "EVENT_REPLAYED", actor_id, reason, detail={"inbox_id": inbox_id, "waits_resolved": resolved},
    )
    return {"success": True, "inbox_id": inbox_id, "status": "PROCESSED", "waits_resolved": resolved}


def enqueue_outbox(run_id: Optional[str], event_type: str, idempotency_key: str,
                   payload: Dict[str, Any], *, max_attempts: int = 5) -> str:
    if not str(event_type or "").strip() or not str(idempotency_key or "").strip():
    ...
    try:
        connection.execute("INSERT INTO GRAPH_OUTBOX (OUTBOX_ID, RUN_ID, EVENT_TYPE, IDEMPOTENCY_KEY, PAYLOAD_JSON, MAX_ATTEMPTS) VALUES (:outbox_id, :run_id, :event_type, :idempotency_key, :payload_json, :max_attempts)", {"outbox_id": outbox_id, "run_id": run_id, "event_type": event_type, "idempotency_key": idempotency_key, "payload_json": _canonical(safe_payload), "max_attempts": maximum})
    except Exception as exc:
        if not _is_unique_violation(exc):
            raise
...
def put_artifact(content: bytes, owner_ref: str, media_type: str = "application/octet-stream", classification: str = "INTERNAL") -> Dict[str, Any]:
    ...
    if not str(owner_ref or "").strip():
        raise ValueError("artifact owner_ref is required")
    ...
    _record_governance_event(
        "ARTIFACT_CREATED", str(owner_ref), "Artifact created", artifact_id=artifact_id,
        detail={"content_hash": content_hash, "content_size": len(content), "classification": classification},
    )
...
def set_artifact_retention(artifact_id: str, retention_until: Optional[Any], actor_id: str,
    ...
    normalized = _db_timestamp(retention_until)
    changed = connection.execute(
        "UPDATE GRAPH_ARTIFACTS SET RETENTION_UNTIL = :retention_until WHERE ARTIFACT_ID = :artifact_id",
        {"artifact_id": artifact_id, "retention_until": normalized},
    ) > 0
    if changed:
        _record_governance_event(
            "ARTIFACT_RETENTION_CHANGED", actor_id, reason, artifact_id=artifact_id,
            detail={"retention_until": normalized.isoformat() if isinstance(normalized, datetime) else normalized},
        )
    return changed
...
def set_artifact_legal_hold(artifact_id: str, enabled: bool, actor_id: str, reason: str) -> bool:
    ...
    changed = connection.execute(sql, {"artifact_id": artifact_id, "actor_id": actor_id, "reason": str(reason)[:2000]}) > 0
    if changed:
        _record_governance_event(
            "ARTIFACT_LEGAL_HOLD" if enabled else "ARTIFACT_LEGAL_HOLD_RELEASED",
            actor_id, reason, artifact_id=artifact_id, detail={"enabled": bool(enabled)},
        )
    return changed
...
def purge_expired_artifacts(actor_id: str, limit: int = 100) -> int:
    ...
    candidates = connection.execute_query(
        "SELECT ARTIFACT_ID FROM GRAPH_ARTIFACTS WHERE RETENTION_UNTIL IS NOT NULL "
        "AND RETENTION_UNTIL <= CURRENT_TIMESTAMP AND LEGAL_HOLD = 'N' "
    ...
    for candidate in candidates[:deleted]:
        _record_governance_event(
            "ARTIFACT_PURGED", actor_id, "Retention period expired",
            artifact_id=candidate.get("artifact_id"), detail={"retention_purge": True},
        )
    return deleted
