"""AI Agent Infra v4.1.0 - Enterprise Edition - Workspace API
...
def create_handoff_session(
    ...
    session_id_sql = "'SES_' || AI_NEW_ID()"
    active_value = "TRUE" if DATABASE_DIALECT == "postgresql" else "'Y'"
    start_column = "CREATED_AT" if DATABASE_DIALECT == "postgresql" else "START_TIME"
    sql = f"""
        INSERT INTO AGENT_SESSION (SESSION_ID, AGENT_ID, WORKSPACE_ID,
                                   PREDECESSOR_SESSION_ID, OWNER_USER_ID,
                                   IS_ACTIVE, {start_column}, CONTEXT, BRANCH_ID)
        VALUES ({session_id_sql}, :aid, :wid, :pred, :owner,
                {active_value}, CURRENT_TIMESTAMP, :ctx, :vbrid)
...
def recover_workspace(workspace_id: str) -> Dict[str, Any]:
    ...
    if DATABASE_DIALECT == "postgresql":
        ...
        SELECT SESSION_ID, AGENT_ID, WORKSPACE_ID, PREDECESSOR_SESSION_ID,
               ...
               TO_CHAR(CREATED_AT, 'YYYY-MM-DD HH24:MI:SS') AS START_TIME,
               CASE WHEN IS_ACTIVE THEN NULL
                    ELSE TO_CHAR(LAST_ACTIVE_AT, 'YYYY-MM-DD HH24:MI:SS') END AS END_TIME,
               CONTEXT
        FROM AGENT_SESSION
        WHERE WORKSPACE_ID = :wid
        ORDER BY CREATED_AT DESC
...
def link_task_to_workspace(workspace_id: str, plan_id: str) -> bool:
    ...
    if DATABASE_DIALECT == "postgresql":
        sql = "INSERT INTO WORKSPACE_TASKS (WORKSPACE_ID, PLAN_ID) VALUES (:wid, :pid)"
    else:
        sql = """
...
def get_workspace_tasks(workspace_id: str) -> List[Dict[str, Any]]:
    ...
    assigned_at = (
        "TO_CHAR(tp.CREATED_AT, 'YYYY-MM-DD HH24:MI:SS')"
        if DATABASE_DIALECT == "postgresql"
        else "TO_CHAR(wt.ASSIGNED_AT, 'YYYY-MM-DD HH24:MI:SS')"
    )
    sql = f"""
        SELECT tp.PLAN_ID, tp.GOAL, tp.STATUS,
