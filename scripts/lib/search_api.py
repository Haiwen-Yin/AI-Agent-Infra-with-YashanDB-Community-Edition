...
- "fulltext"     - YashanDB SEARCH INDEX CONTAINS+SCORE (best for exact keyword/phrase search)
...
STRATEGIES = {
    ...
    "fulltext": {
        "name": "YashanDB SEARCH INDEX Full-Text Search",
        "description": "YashanDB SEARCH INDEX CONTAINS with SCORE ranking. Best for exact keyword/phrase search with linguistic features (stemming, boolean, fuzzy).",
        "signals": ["fulltext"],
    ...
    "keyword": {
        ...
        "description": "Simple SQL LIKE pattern matching on title/content. Best for partial match, wildcard, or when YashanDB SEARCH INDEX index is unavailable.",
        "signals": ["keyword"],
    ...
    "hybrid": {
        ...
        "description": "Combines vector similarity with YashanDB SEARCH INDEX scoring. Best for general-purpose search needing both semantic and lexical signals.",
...
def _get_strategy_params(strategy: str) -> List[Dict[str, str]]:
    params = {
        ...
        "fulltext": [
            {"name": "query", "type": "str", "required": "True", "description": "YashanDB SEARCH INDEX query (supports AND/OR/NOT, $fuzzy, stemming)"},
