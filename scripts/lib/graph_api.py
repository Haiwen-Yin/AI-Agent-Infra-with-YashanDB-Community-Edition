"""AI Agent Infra v4.4.11 - Property Graph API
...
def _edge_id_expr(alias: str, column: str) -> str:
    """Normalize edge IDs without failing on legacy PostgreSQL strings.

    PostgreSQL keeps edge endpoints as text for compatibility with older
    installations, while current ``ENTITIES.ENTITY_ID`` values are numeric.
    Comparing their text representations matches numeric endpoints and safely
    ignores historical values such as ``PG_AGENT_001`` that have no entity row.
    """
    reference = f"{alias}.{column}"
    if DATABASE_DIALECT == "postgresql":
        return f"CAST({reference} AS TEXT)"
    return reference


def _edge_id_bind(name: str) -> str:
    if DATABASE_DIALECT == "postgresql":
        return f"CAST(:{name} AS TEXT)"
    return f":{name}"


def _entity_id_expr(alias: str) -> str:
    """Use the same comparison domain for entity and legacy edge IDs."""
    reference = f"{alias}.ENTITY_ID"
    if DATABASE_DIALECT == "postgresql":
        return f"CAST({reference} AS TEXT)"
    return reference
...
def _relational_neighbors(entity_id, direction, edge_type, min_strength, limit,
    ...
    if direction in ("outgoing", "both"):
        ...
        target_join = f"{_entity_id_expr('target')} = {_edge_id_expr('edge', 'TARGET_ID')}"
    ...
    if direction in ("incoming", "both"):
        edge_target = f"{_edge_id_expr('edge', 'TARGET_ID')} = {_edge_id_bind('eid')}"
        source_join = f"{_entity_id_expr('source')} = {_edge_id_expr('edge', 'SOURCE_ID')}"
...
def get_subgraph(
    ...
    source_filter = ", ".join(_edge_id_bind(f"sid{i}") for i in range(len(entity_ids)))
    target_filter = ", ".join(_edge_id_bind(f"tid{i}") for i in range(len(entity_ids)))
    edges = execute_query(f"""
        SELECT e.EDGE_ID, e.SOURCE_ID, e.SOURCE_TYPE, e.TARGET_ID,
               e.EDGE_TYPE, e.STRENGTH, e.CONFIDENCE
        FROM ENTITY_EDGES e
        WHERE {_edge_id_expr('e', 'SOURCE_ID')} IN ({source_filter})
           OR {_edge_id_expr('e', 'TARGET_ID')} IN ({target_filter})
...
def find_communities(
    ...
    source_join = f"{_edge_id_expr('e2', 'SOURCE_ID')} = {_entity_id_expr('v')}"
    target_join = f"{_edge_id_expr('e3', 'TARGET_ID')} = {_entity_id_expr('v')}"
    sql = f"""
            ...
            rows = execute_query(f"""
                SELECT edge.EDGE_ID, edge.SOURCE_ID, edge.SOURCE_TYPE,
                  ...
                  LEFT JOIN ENTITIES entity
                    ON {_entity_id_expr('entity')} = {_edge_id_expr('edge', 'SOURCE_ID')}
                   AND entity.ENTITY_TYPE = edge.SOURCE_TYPE
                 WHERE {_edge_id_expr('edge', 'TARGET_ID')} = {_edge_id_bind('target')}
...
def find_contradictions(entity_id: str) -> List[Dict[str, Any]]:
    source_id_expr = _edge_id_expr("edge", "SOURCE_ID")
    target_id_expr = _edge_id_expr("edge", "TARGET_ID")
    entity_id_bind = _edge_id_bind("eid")
    rows = execute_query(f"""
        SELECT edge.EDGE_ID, edge.SOURCE_ID, edge.SOURCE_TYPE, edge.TARGET_ID,
          ...
          FROM ENTITY_EDGES edge
          LEFT JOIN ENTITIES entity
            ON {_entity_id_expr('entity')} = CASE WHEN {source_id_expr} = {entity_id_bind}
                                                  THEN {target_id_expr} ELSE {source_id_expr} END
         WHERE edge.EDGE_TYPE = 'CONTRADICTS'
           AND ({source_id_expr} = {entity_id_bind} OR {target_id_expr} = {entity_id_bind})
         ORDER BY edge.STRENGTH DESC
    """, {"eid": entity_id})
    return [_row_to_dict(row) for row in rows]
