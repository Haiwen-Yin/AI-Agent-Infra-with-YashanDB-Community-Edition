"""AI Agent Infra v4.4.11 - Community Edition - Loop Engineering API
...
from .connection import (
    ...
    execute_insert_returning_id, scalar_select_suffix,
)
...
def create_loop(
    ...
    entity_id = execute_insert_returning_id("""
        ...
        VALUES (AI_NEW_ID(), 'LOOP_DEFINITION', :title, :summary, 'ACTIVE',
    ...
    try:
        ...
        graph_compat.ensure_loop_graph(get_loop(entity_id) or {
            "loop_id": entity_id, "title": title, "goal_definition": goal_definition,
            "stop_conditions": stop_conditions, "evaluation_config": evaluation_config,
            "trigger_config": trigger_config, "owned_by_agent": owned_by_agent,
        })
    except ImportError:
        pass
    return entity_id
...
def update_loop(loop_id: str, **kwargs: Any) -> bool:
    ...
    for k, v in kwargs.items():
        if k in entity_fields and v is not None:
            ...
            params[k] = v
    if sets:
        execute(f"UPDATE ENTITIES SET {', '.join(sets)}, UPDATED_AT = CURRENT_TIMESTAMP "
    ...
    if count:
        try:
            from . import graph_compat
            graph_compat.ensure_loop_graph(get_loop(loop_id) or {"loop_id": loop_id})
        except ImportError:
            pass
    return count > 0
...
def start_run(loop_id: str, agent_id: str, trigger_type: str = "MANUAL",
    ...
    run_id = execute_insert_returning_id("""
        INSERT INTO LOOP_RUNS (RUN_ID, LOOP_ID, AGENT_ID, TRIGGER_TYPE, TRIGGER_SOURCE,
                               ...
                               PARENT_RUN_ID)
        VALUES (AI_NEW_ID(), :loop_id, :agent_id, :trigger_type, :trigger_source,
                'RUNNING', 0, 0, CURRENT_TIMESTAMP, :parent_run_id)
    ...
    try:
        from . import graph_compat
        graph_compat.start_loop_run(
            {"run_id": run_id, "agent_id": agent_id},
            get_loop(loop_id) or {"loop_id": loop_id, "owned_by_agent": agent_id},
            agent_id or "system",
        )
    except ImportError:
        pass
...
def pause_run(run_id: str) -> bool:
    run = get_run(run_id)
    changed = execute("UPDATE LOOP_RUNS SET STATUS = 'PAUSED' "
                      "WHERE RUN_ID = :id AND STATUS = 'RUNNING'", {"id": run_id}) > 0
    if changed and run:
        try:
            from . import graph_compat
            graph_compat.pause_legacy_run("LOOP_RUN", run_id, run.get("agent_id") or "system", "Legacy Loop paused")
        except ImportError:
            pass
    return changed
...
def resume_run(run_id: str) -> bool:
    run = get_run(run_id)
    changed = execute("UPDATE LOOP_RUNS SET STATUS = 'RUNNING' "
                      "WHERE RUN_ID = :id AND STATUS = 'PAUSED'", {"id": run_id}) > 0
    if changed and run:
        try:
            from . import graph_compat
            graph_compat.resume_legacy_run("LOOP_RUN", run_id, run.get("agent_id") or "system", "Legacy Loop resumed")
        except ImportError:
            pass
    return changed
...
def stop_run(run_id: str, reason: Optional[str] = None) -> bool:
    ...
    result = execute("UPDATE LOOP_RUNS SET STATUS = 'STOPPED', FINAL_RESULT = :reason, "
                     "COMPLETED_AT = CURRENT_TIMESTAMP WHERE RUN_ID = :id "
    ...
    if result:
        ...
        try:
            from . import graph_compat
            graph_compat.finish_legacy_run("LOOP_RUN", run_id, run.get("agent_id") or "system", reason or "Legacy Loop stopped")
        except ImportError:
            pass
    return result
...
def fail_run(run_id: str, error_message: str) -> bool:
    ...
    if result and run:
        ...
        try:
            from . import graph_compat
            graph_compat.finish_legacy_run("LOOP_RUN", run_id, run.get("agent_id") or "system", error_message or "Legacy Loop failed")
        except ImportError:
            pass
    return result
...
def complete_run(run_id: str, final_result: Optional[str] = None) -> bool:
    ...
    if result:
        ...
        try:
            from . import graph_compat
            graph_compat.finish_legacy_run("LOOP_RUN", run_id, run.get("agent_id") or "system", final_result or "Legacy Loop completed", success=True)
        except ImportError:
            pass
    return result
...
def bind_loop_to_step(loop_id: str, step_id: str,
    ...
    binding_id = execute_insert_returning_id("""
        ...
        VALUES (AI_NEW_ID(), :loop_id, :step_id, :binding_type,
                :auto_start, CURRENT_TIMESTAMP)
...
def on_loop_run_completed(run_id: str) -> List[str]:
    ...
    updated = []
    for r in rows:
        execute("UPDATE TASK_STEPS SET STATUS = 'SUCCESS', COMPLETED_AT = CURRENT_TIMESTAMP "
...
def record_iteration(
    ...
    iter_id = execute_insert_returning_id("""
        ...
        VALUES (AI_NEW_ID(), :run_id, :iter_order,
                :plan_data, :actions, :observations,
                :eval_result, :eval_passed, :adjustment,
                :tokens, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ...
    if evaluation_passed:
        iteration_text = (
            "CAST(ITERATION_COUNT AS TEXT)"
            if DATABASE_DIALECT == "postgresql"
            else "TO_CHAR(ITERATION_COUNT)"
        )
        execute("UPDATE LOOP_RUNS SET STATUS = 'COMPLETED', COMPLETED_AT = CURRENT_TIMESTAMP, "
                f"FINAL_RESULT = 'Goal achieved at iteration ' || {iteration_text} "
                "WHERE RUN_ID = :id", {"id": run_id})
    try:
        from . import graph_compat
        graph_compat.sync_loop_iteration(
            run_id,
            {
                ...
                "status": "SUCCESS" if evaluation_passed else "RUNNING",
                "token_count": token_usage,
            },
            not evaluation_passed,
            str(run.get("agent_id") or "system"),
        )
    except ImportError:
        pass
    return iter_id
...
def get_loop_stats(loop_id: str) -> Dict[str, Any]:
    scalar_source = scalar_select_suffix()
    try:
        row = execute_query_one(f"""
            SELECT
                ...
                (SELECT COALESCE(SUM(TOTAL_TOKENS), 0) FROM LOOP_RUNS WHERE LOOP_ID = :id) AS total_tokens{scalar_source}
        """, {"id": loop_id})
...
def add_hook(loop_id: str, hook_event: str, hook_type: str,
    ...
    return execute_insert_returning_id("""
        ...
        VALUES (AI_NEW_ID(), :loop_id, :event, :type,
                :config, :priority, 'Y', CURRENT_TIMESTAMP)
...
def cleanup_old_runs(days_threshold: int = 90) -> int:
    try:
        ...
        try:
            n1 = execute("""DELETE FROM LOOP_ITERATIONS WHERE RUN_ID IN (
                SELECT RUN_ID FROM LOOP_RUNS WHERE STATUS IN ('COMPLETED','STOPPED','FAILED','TIMEOUT')
                  AND COMPLETED_AT < CURRENT_TIMESTAMP - :days)""", {"days": days_threshold})
            n2 = execute("""DELETE FROM LOOP_RUNS WHERE STATUS IN ('COMPLETED','STOPPED','FAILED','TIMEOUT')
                  AND COMPLETED_AT < CURRENT_TIMESTAMP - :days""", {"days": days_threshold})
        except Exception as e:
            import logging
...
def _eval_test(cfg: Dict, iter_data: Dict) -> Dict[str, Any]:
    """Queue test execution for an approval-gated worker."""
    import shlex
    from .connection import get_current_agent_id
    from .execution_control import enqueue_job

    argv = cfg.get("eval_argv")
    if not argv and cfg.get("eval_command"):
        argv = shlex.split(cfg["eval_command"])
    if not argv:
        return {"passed": False, "error": "No eval_argv configured"}
    job = enqueue_job(
        "LOOP_TEST",
        {"argv": argv, "timeout": cfg.get("eval_timeout", 120), "cwd": cfg.get("cwd", ".")},
        get_current_agent_id() or iter_data.get("agent_id") or "system",
        idempotency_key="loop-test:{}:{}".format(
            iter_data.get("iteration_id", ""),
            hashlib.sha256(json.dumps(cfg, sort_keys=True).encode()).hexdigest(),
        ),
    )
    return {"passed": False, "pending": True, "eval_type": "TEST", "job_id": job.get("job_id")}


def _eval_diff(cfg: Dict, iter_data: Dict) -> Dict[str, Any]:
    """Queue git diff evaluation for an approval-gated worker."""
    import shlex
    from .connection import get_current_agent_id
    from .execution_control import enqueue_job

    argv = cfg.get("diff_argv") or shlex.split(cfg.get("diff_command", "git diff --stat"))
    job = enqueue_job(
        "LOOP_DIFF", {"argv": argv, "timeout": 60, "cwd": cfg.get("cwd", ".")},
        get_current_agent_id() or iter_data.get("agent_id") or "system",
        idempotency_key="loop-diff:{}:{}".format(
            iter_data.get("iteration_id", ""),
            hashlib.sha256(json.dumps(cfg, sort_keys=True).encode()).hexdigest(),
        ),
    )
    return {"passed": False, "pending": True, "eval_type": "DIFF", "job_id": job.get("job_id")}
...
def execute_loop_iteration(run_id: str, agent_id: str,
    ...
    if stop_check != "CONTINUE":
        if stop_check == "TIMEOUT":
            execute("UPDATE LOOP_RUNS SET STATUS='TIMEOUT', COMPLETED_AT=CURRENT_TIMESTAMP "
                    "WHERE RUN_ID=:id", {"id": run_id})
    ...
    if passed:
        ...
        try:
            ...
            graph_compat.finish_legacy_run(
                "LOOP_RUN", run_id, agent_id, "Loop evaluation passed", success=True
            )
        except ImportError:
            pass
    ...
    if stop_check != "CONTINUE":
        if stop_check == "TIMEOUT":
            ...
            execute("UPDATE LOOP_RUNS SET STATUS='STOPPED', COMPLETED_AT=CURRENT_TIMESTAMP "
                    "WHERE RUN_ID=:id", {"id": run_id})
        try:
            from . import graph_compat
            graph_compat.finish_legacy_run(
                "LOOP_RUN", run_id, agent_id,
                "Loop iteration stop condition reached", success=False,
            )
        except ImportError:
            pass
